...
void* Mesh::CombineBufferData(size_t &size)
{

	size = 0;

	for (auto &b : geometryBuffers)
	{
		size += b.second.buffer->getSize();
	}

	char* dataToReturn;
	dataToReturn = new char[size];
	size_t offset = 0;
	for (auto &b : geometryBuffers)
	{
		memcpy((dataToReturn + offset), b.second.buffer->getData(), b.second.buffer->getSize());
		offset += b.second.buffer->getSize();
	}
